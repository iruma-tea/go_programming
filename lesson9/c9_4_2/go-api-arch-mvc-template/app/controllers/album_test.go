...
func (suite *AlbumControllersSuite) TestGet() {
	createAlbum, _ := models.CreateAlbum("test", time.Now(), "sports")

	request, _ := api.NewGetAlbumByIdRequest("/api/v1", createAlbum.ID)

	w := httptest.NewRecorder()
	ginContext, _ := gin.CreateTestContext(w)
	ginContext.Request = request
	suite.albumHandler.GetAlbumById(ginContext, createAlbum.ID)
	...
	suite.Assert().Equal("test", albumGetResponse.Title)
	suite.Assert().Equal("sports", string(albumGetResponse.Category.Name))
	suite.Assert().NotNil(albumGetResponse.ReleaseDate.Date())
}
...
func (suite *AlbumControllersSuite) TestUpdate() {
	...
	requeset, _ := api.NewUpdateAlbumByIdRequest("/api/v1", createdAlbum.ID, api.UpdateAlbumByIdJSONRequestBody{
		Title:    &title,
		Category: &category,
	})
	w := httptest.NewRecorder()
	ginContext, _ := gin.CreateTestContext(w)
	ginContext.Request = requeset
...
func (suite *AlbumControllersSuite) TestUpdateNoAlbumFailure() {
	...
	request, _ := api.NewUpdateAlbumByIdRequest("/api/v1", doesNotExistAlbumID, api.UpdateAlbumByIdJSONRequestBody{
		Title:    &title,
		Category: &category,
	})
	w := httptest.NewRecorder()
...
func (suite *AlbumControllersSuite) TestUpdateFailure() {
	...
	request, _ := api.NewUpdateAlbumByIdRequest("/api/v1", 1, api.UpdateAlbumByIdJSONRequestBody{
		Title:    &title,
		Category: &category,
	})
	w := httptest.NewRecorder()
